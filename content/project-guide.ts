    ...
    steps: [
      {
        ...
        points: [
          ...
          { en: `Before defining anything, he modeled a step this framework doesn't list: real market research — are there competitors, what do they offer — "so you don't spend two months building something that turns out to already exist."`, he: `לפני שמגדירים משהו, הוא הדגים שלב שהמסגרת הזו לא כוללת: מחקר שוק אמיתי — האם יש מתחרים, מה הם מציעים — "כדי שלא תעבדי חודשיים על משהו שמתברר שכבר קיים."` },
          { en: `The actual problem statement he reached for the herbal project, live: "I heard herb X helps with my condition — is that claim actually evidence-based?" A patient or caregiver has no reliable way to check.`, he: `הגדרת הבעיה בפועל שהוא הגיע אליה, בשידור חי, עבור פרויקט הצמחים: "שמעתי שצמח X עוזר למחלה שלי — האם הטענה הזו באמת מבוססת?" למטופלת או למטפל אין דרך אמינה לבדוק.` },
          { en: `The safety constraint that shaped the whole product, said directly: a plant being researched doesn't mean it's beneficial — showing a bare list of researched herbs risks being read as a recommendation. That risk had to shape the product's structure from the start, not get resolved later.`, he: `אילוץ הבטיחות שעיצב את המוצר כולו, נאמר במפורש: צמח שנחקר לא אומר שהוא מועיל — הצגת רשימה גולמית של צמחים שנחקרו מסתכנת בכך שתיקרא כהמלצה. הסיכון הזה היה חייב לעצב את מבנה המוצר מההתחלה, לא להיפתר בהמשך.` },
        ],
      },
      {
        number: 2,
        ...
        points: [
          ...
          { en: `The real user for the herbal project, per the repo's own README: a person with cancer, or their caregiver — a family member, per the screen flow walked through live in Session 22. Hebrew-first (RTL), built for an Israeli audience. What blocks them today: mixed, unsourced claims about herbal remedies online, with no single place that grades the evidence AND has a human researcher check it before trusting a personal answer. The real user story: as a cancer patient or caregiver, I want to check whether a specific herb genuinely helps with appetite improvement, so that I can trust the answer because a person reviewed it — not just raw AI output.`, he: `המשתמש האמיתי של פרויקט הצמחים, לפי ה-README של המאגר עצמו: אדם עם סרטן, או המטפל/ת שלו — בן משפחה, לפי זרימת המסך שהוצגה בשידור חי במפגש 22. עברית קודם (RTL), נבנה לקהל ישראלי. מה חוסם אותם היום: טענות מעורבות וחסרות-מקור על תרופות צמחיות באינטרנט, בלי מקום אחד שמדרג את הראיות וגם יש בו חוקר אנושי שבודק לפני שסומכים על תשובה אישית. סיפור המשתמש האמיתי: בתור מטופלת סרטן או מטפלת, אני רוצה לבדוק אם צמח מסוים באמת עוזר לשיפור תיאבון, כדי שאוכל לסמוך על התשובה כי אדם בדק אותה — לא רק פלט AI גולמי.` },
        ],
      },
      {
        number: 3,
        ...
        points: [
          ...
          { en: `The real V1 boundary for the herbal project, narrowed live in class from a long symptom brainstorm (cancer, winter illness, children's illness, immune support, pain, nausea, fatigue, sleep) down to a single one: a claim about one herb and appetite improvement — stated explicitly in the repo's own README. Out of scope, in the README's own words: the platform does not recommend which herb to take, does not prescribe doses, and does not approve combinations with treatment. Also out of v1, per docs/decisions.md: automated researcher matching (D-005 — admin assigns manually for now, a simple replaceable column) and request-update emails (D-006).`, he: `הגבול האמיתי של V1 לפרויקט הצמחים, שצומצם בשידור חי בכיתה מסיעור מוחות ארוך של תסמינים (סרטן, מחלות חורף, מחלות ילדים, חיזוק חיסוני, כאב, בחילה, עייפות, שינה) לאחד בלבד: טענה על צמח אחד ושיפור תיאבון — נאמר במפורש ב-README של המאגר עצמו. מחוץ לתחום, במילותיו של ה-README: הפלטפורמה לא ממליצה איזה צמח לקחת, לא רושמת מינונים, ולא מאשרת שילובים עם טיפול. גם מחוץ ל-v1, לפי docs/decisions.md: התאמת חוקר אוטומטית (D-005 — מנהל מקצה ידנית בינתיים, עמודה פשוטה הניתנת להחלפה) והתראות אימייל על עדכוני בקשה (D-006).` },
        ],
      },
    ],
  },
    ...
    steps: [
      {
        ...
        points: [
          ...
          { en: `The real journey he actually generated live for the herbal project (Session 22 transcript): family member → login → submit a request to check a plant → brief clarification if the claim is ambiguous → request enters a queue, user sees a waiting status → AI prepares a draft (research search included) → draft with findings ready → researcher's decision — correction needed, or approve for publishing → personal answer appears in the user's account → the version is saved for a future update.`, he: `המסע האמיתי שהוא בפועל ייצר בשידור חי עבור פרויקט הצמחים (תמליל מפגש 22): בן משפחה ← כניסה לחשבון ← הגשת בקשה לבדיקת צמח ← בירור קצר במקרה שהטענה עמומה ← הבקשה נכנסת לתור, המשתמש רואה סטטוס המתנה ← AI מכין טיוטה (כולל חיפוש מחקרי) ← יש טיוטה עם ממצאים ← החלטת החוקר — נדרש תיקון, או אישור לפרסום ← תשובה אישית מופיעה בחשבון המשתמש ← הגרסה נשמרת לעדכון עתידי.` },
        ],
      },
      {
        number: 5,
        ...
        points: [
          ...
          { en: `The real design system built for the herbal project, from docs/design/stitch.md — Stitch project "Herbal Evidence" (private), design system asset "Herbal Evidence – Calm Research": light theme, primary color #3F6B5A (a calm, muted green), neutral #F7F7F4, Noto Sans typeface, 8px corner radius throughout.`, he: `מערכת העיצוב האמיתית שנבנתה לפרויקט הצמחים, מ-docs/design/stitch.md — פרויקט Stitch בשם "Herbal Evidence" (פרטי), נכס מערכת עיצוב "Herbal Evidence – Calm Research": ערכת נושא בהירה, צבע ראשי #3F6B5A (ירוק מושקט ומעודן), ניטרלי #F7F7F4, גופן Noto Sans, רדיוס פינות 8px לכל אורך.` },
        ],
      },
    ],
  },
