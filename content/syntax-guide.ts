    ...
    topics: [
      {
        ...
        points: [
          { en: `\`SELECT columns FROM table WHERE condition\` — reads as a full sentence, recognizable from the English words alone, before any formal rule is taught.`, he: `\`SELECT columns FROM table WHERE condition\` — נקראת כמשפט מלא, ניתנת לזיהוי מהמילים באנגלית בלבד, לפני שנלמד כלל פורמלי אחד.` },
          { en: `Written order vs. evaluation order — fixed syntax isn't the same as how the engine actually resolves a query; Resolution order — \`FROM\` first, then \`WHERE\`, and only at the end does \`SELECT\` decide which columns to display.`, he: `סדר הכתיבה מול סדר ההערכה — תחביר קבוע הוא לא אותו דבר כמו איך שהמנוע בפועל פותר שאילתה; סדר הפתרון — \`FROM\` קודם, אחר כך \`WHERE\`, ורק בסוף \`SELECT\` מחליטה אילו עמודות להציג.` },
          { en: `\`CREATE TABLE ... (...)\` — creates a table; \`INSERT INTO table VALUES (...)\` — adds a row; \`UPDATE table SET col = value WHERE ...\` — changes existing rows; \`DELETE FROM table WHERE ...\` — removes rows.`, he: `\`CREATE TABLE ... (...)\` — יוצרת טבלה; \`INSERT INTO table VALUES (...)\` — מוסיפה שורה; \`UPDATE table SET col = value WHERE ...\` — משנה שורות קיימות; \`DELETE FROM table WHERE ...\` — מסירה שורות.` },
        ],
        keyTakeaway: {
      ...
      },
      {
        ...
        points: [
          { en: `Foreign key value — points to another table's row by its \`id\`, not to a final answer; \`address_id = 2\` — the address row whose own \`id\` is 2, and that row's \`city_id\` is what actually names the city; Skipping a hop and matching digits by eye — how a whole class gets it wrong.`, he: `ערך מפתח זר — מצביע על שורה בטבלה אחרת לפי ה-\`id\` שלה, לא על תשובה סופית; \`address_id = 2\` — שורת הכתובת שה-\`id\` שלה עצמה הוא 2, ועמודת \`city_id\` של אותה שורה היא זו שבאמת קובעת את העיר; דילוג על קפיצה והתאמת ספרות במבט — הדרך שכיתה שלמה טועה.` },
          { en: `Normalization — splitting repeated data into its own small table, referenced by a foreign key. A typo repeated across 200 rows needs 200 fixes in one big table, but exactly one fix once normalized.`, he: `נרמול — פיצול נתונים חוזרים לטבלה קטנה משלהם, מופנית אליה במפתח זר. טעות הקלדה שחוזרת ב-200 שורות דורשת 200 תיקונים בטבלה גדולה אחת, אבל בדיוק תיקון אחד אחרי שנרמל.` },
          { en: `\`CREATE TABLE\` review — check column definitions, not just table names: is \`id\` marked primary key and set to auto-increment, and does each foreign key reference the right table and column?`, he: `בדיקת \`CREATE TABLE\` — בודקים את הגדרות העמודות, לא רק את שמות הטבלאות: האם \`id\` מסומן כמפתח ראשי ומוגדר להעלות את עצמו אוטומטית, והאם כל מפתח זר מפנה לטבלה ולעמודה הנכונות?` },
        ],
      },
      {
        number: 'sql-04',
  ...
  },
  {
    ...
    topics: [
      ...
      },
      {
        ...
        source: 'Dr. Zuari · CLI Reference Deck',
        points: [
          { en: `\`pwd\` — prints the current directory (Unix); \`cd\` with no arguments — does the same on Windows; \`ls\` (Unix) / \`dir\` (Windows) — lists everything in the current folder.`, he: `\`pwd\` — מדפיסה את התיקייה הנוכחית (Unix); \`cd\` בלי ארגומנטים — עושה את אותו הדבר ב-Windows; \`ls\` (Unix) / \`dir\` (Windows) — מציגה רשימה של הכול בתיקייה הנוכחית.` },
          { en: `\`cd <folder>\` — moves into a folder, e.g. \`cd Documents\`; \`cd Documents/my_project\` — enters a nested folder in one step; \`cd ..\` — goes up one level (\`cd ../..\` goes up two); \`cd ~\` — jumps straight to the home directory (Unix); \`Tab\` — autocompletes folder names; \`pwd\` — confirms where you are.`, he: `\`cd <folder>\` — עוברת לתוך תיקייה, למשל \`cd Documents\`; \`cd Documents/my_project\` — נכנסת לתיקייה מקוננת בצעד אחד; \`cd ..\` — עולה רמה אחת (\`cd ../..\` עולה שתי רמות); \`cd ~\` — קופצת ישר לתיקיית הבית (Unix); \`Tab\` — משלים אוטומטית שמות תיקיות; \`pwd\` — מאשרת את המיקום הנוכחי.` },
          { en: `Environment variables — key-value pairs that store config (paths, API keys) for programs; \`export MY_KEY="abc123"\` — sets one for the session (Mac/Linux); \`set MY_KEY=abc123\` — same, Windows CMD; \`$env:MY_KEY = "abc123"\` — same, Windows PowerShell.`, he: `משתני סביבה — זוגות מפתח-ערך ששומרים קונפיגורציה (נתיבים, מפתחות API) עבור תוכניות; \`export MY_KEY="abc123"\` — הגדרה לסשן הנוכחי (Mac/Linux); \`set MY_KEY=abc123\` — אותו דבר, Windows CMD; \`$env:MY_KEY = "abc123"\` — אותו דבר, Windows PowerShell.` },
          { en: `\`os.environ['MY_KEY']\` — reads an environment variable in Python; \`os.environ.get('MY_KEY', 'default')\` — the safer form, falls back to a default instead of raising an error when the key is missing.`, he: `\`os.environ['MY_KEY']\` — קוראת משתנה סביבה בפייתון; \`os.environ.get('MY_KEY', 'default')\` — הצורה הבטוחה יותר, נופלת לברירת מחדל במקום לזרוק שגיאה כשהמפתח חסר.` },
        ],
      },
      {
        number: 'cli-03',
      ...
      },
      {
        ...
        points: [
          { en: `\`git add <file>\` — stage a file, the CLI equivalent of GitHub Desktop's staging step; \`git commit -m "message"\` — commit, never skip the message; \`git push\` — push, syncs to the cloud copy.`, he: `\`git add <file>\` — stage, המקבילה ב-CLI לשלב ה-staging של GitHub Desktop; \`git commit -m "message"\` — commit, לעולם לא לדלג על ההודעה; \`git push\` — push, מסנכרן לעותק הענן.` },
          ...
          { en: `\`git pull\` — fetches and merges the latest changes from the cloud copy, the opposite direction of \`git push\`; \`git log\` — reads a version's history; \`git diff\` — reads exactly what changed in a version.`, he: `\`git pull\` — מושכת וממזגת את השינויים האחרונים מהעותק בענן, הכיוון ההפוך מ-\`git push\`; \`git log\` — קוראת את ההיסטוריה של גרסה; \`git diff\` — קוראת בדיוק מה השתנה בגרסה.` },
        ],
        keyTakeaway: {
    ...
    topics: [
      {
        ...
        points: [
          { en: `Prerequisite — Node.js 18+, downloaded from nodejs.org (Windows uses the .msi installer); Install — \`npm install -g @anthropic-ai/claude-code\`; Verify — \`claude --version\`.`, he: `דרישה מקדימה — Node.js 18+, הורדה מ-nodejs.org (Windows משתמש במתקין .msi); התקנה — \`npm install -g @anthropic-ai/claude-code\`; אימות — \`claude --version\`.` },
          { en: `CLI — standalone terminal tool, use CMD on Windows, not PowerShell; VS Code Extension — integrated into the editor, adds session forking and usage indicators.`, he: `CLI — כלי טרמינל עצמאי, משתמשים ב-CMD ב-Windows, לא PowerShell; תוסף VS Code — משולב בעורך, מוסיף פיצול session ומחווני שימוש.` },
          { en: `\`claude\` — launches a session from inside any project folder; Option+P (Mac) / Alt+P (Windows) — switches models mid-prompt.`, he: `\`claude\` — מפעילה session מתוך תיקיית פרויקט כלשהי; Option+P (Mac) / Alt+P (Windows) — מחליפה מודלים באמצע prompt.` },
        ],
      },
      {
        number: 'claude-02',
        ...
        source: 'Dr. Zuari · "claude code workshop.pdf"',
        points: [
          { en: `CLAUDE.md — a config file Claude reads automatically every session, persistent project context with higher authority than an ordinary prompt (system rules, not suggestions); \`/init\` — auto-generates one for any existing project, to then customize.`, he: `CLAUDE.md — קובץ קונפיגורציה ש-Claude קורא אוטומטית בכל session, הקשר פרויקט מתמיד עם סמכות גבוהה יותר מ-prompt רגיל (חוקי מערכת, לא הצעות); \`/init\` — מייצר אחד אוטומטית לכל פרויקט קיים, כדי להתאים אותו אישית אחר כך.` },
          { en: `Karpathy's 4 rules for a CLAUDE.md, cited directly in the deck.`, he: `4 הכללים של קרפתי ל-CLAUDE.md, מצוטטים ישירות בחוברת.` },
          { en: `Think Before Coding — define success criteria before writing any code; Keep It Simple — minimum code to solve the problem, no speculative abstraction; Surgical Changes Only — touch only what the request requires; Goal-Driven Verification — confirm the result actually satisfies the success criteria defined up front.`, he: `לחשוב לפני קידוד — להגדיר קריטריוני הצלחה לפני כתיבת קוד; לשמור על פשטות — מינימום קוד לפתרון הבעיה, בלי הפשטה ספקולטיבית; שינויים כירורגיים בלבד — לגעת רק במה שהבקשה דורשת; אימות מונחה-מטרה — לוודא שהתוצאה באמת עומדת בקריטריוני ההצלחה שהוגדרו מראש.` },
      ...
      },
      {
        ...
        points: [
          ...
          { en: `\`/workflows\` — views active Dynamic Workflow runs.`, he: `\`/workflows\` — מציגה ריצות Dynamic Workflow פעילות.` },
          { en: `Every sub-agent — Dynamic Workflows or Ultraplan — starts with an empty, fresh context window; it does not inherit the parent session's history.`, he: `כל תת-סוכן — Dynamic Workflows או Ultraplan — מתחיל עם חלון הקשר ריק וטרי; הוא לא יורש את היסטוריית ה-session ההורה.` },
        ],
      },
      {
        number: 'claude-05',
        ...
        points: [
          ...
          { en: `Skill — a single \`SKILL.md\` reusable instruction set (invoked with \`/skill-name\`); Plugin — bundles skills + agents + hooks + MCP servers into one installable unit; Custom command — lives in \`~/.claude/commands/name.md\`, invoked as \`/name\`.`, he: `מיומנות (Skill) — קובץ \`SKILL.md\` יחיד עם הוראות לשימוש חוזר (מופעל עם \`/skill-name\`); תוסף (Plugin) — אורז מיומנויות + סוכנים + hooks + שרתי MCP ליחידה אחת להתקנה; פקודה מותאמת אישית — חיה ב-\`~/.claude/commands/name.md\`, מופעלת כ-\`/name\`.` },
        ],
      },
    ],
    attachments: [
